...
params <- list(
  anchor_structure = list(
    tss_all = list(
      tss = list(
        filter_1 = list(aname = "ncRNA_MTR4kd", operator = "==", value = TRUE)
      )
    )
  ),
  bait_structure = list(
    enhancer = list(
      h3k4me1 = list(
        combine = "and",
        filter_1 = list(aname = "tsspm10kb", operator = "==", value = FALSE),
        filter_2 = list(aname = "mtr4",      operator = "==", value = TRUE)
      )
    )
  ),
  h5path = list(
...
skip_go <- TRUE
  ...
  rm(h3k4me1_gr, mtr4_gr, tss_gr, tss_pm10kb, prompts_gr, Go_bait, Go_anchor)
